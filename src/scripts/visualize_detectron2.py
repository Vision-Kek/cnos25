...
def visualize(cfg: DictConfig) -> None:
    ...
    for idx, (scene_id, image_id) in tqdm(enumerate(list_scene_id_and_frame_id)):
        ...
        has_mask = dets[0].get('segmentation') is not None and len(dets[0]['segmentation']['size'])>0
        object_ids, scores, bboxes = [], [], []
        masks = [] if has_mask else None
        for det in dets:
            if det['scene_id'] == scene_id and det['image_id'] == image_id:
                if has_mask: masks.append(rle_to_mask(det['segmentation']))
                object_ids.append(det['category_id']-1)
                scores.append(det['score'])
                bbox = det['bbox']
                bbox = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]
                bboxes.append(bbox)
        if has_mask: masks = np.stack(masks)
        scores = np.array(scores)
        bboxes = np.array(bboxes)
        assert len(object_ids) == len(scores) == len(bboxes)
